	include "cpu/68000/include/dsub.inc"
	include "cpu/68000/include/macros.inc"
	include "cpu/68000/include/xy_string.inc"
	include "cpu/68000/include/tests/input.inc"
	include "global/include/screen.inc"

	include "input.inc"
	include "machine.inc"
	include "mad_rom.inc"

	global input_test

	section code

input_test:
		lea	d_screen_xys_list, a0
		RSUB	print_xy_string_list

	.loop_test:

		lea	d_input_list, a0
		jsr	print_input_list

		move.b	REG_INPUT, d0
		not.b	d0
		cmp.b	#(INPUT_B2|INPUT_RIGHT), d0
		bne	.loop_test

		rts

	section data
	align 2

d_input_list:
	INPUT_ENTRY (SCREEN_START_Y + 3), REG_INPUT_P1
	INPUT_ENTRY (SCREEN_START_Y + 4), REG_INPUT_P2
	INPUT_ENTRY (SCREEN_START_Y + 5), REG_INPUT_IN2
	INPUT_ENTRY (SCREEN_START_Y + 6), REG_INPUT_IN3
	INPUT_ENTRY (SCREEN_START_Y + 7), REG_INPUT_DSW1
	INPUT_ENTRY (SCREEN_START_Y + 8), REG_INPUT_DSW2
	INPUT_ENTRY (SCREEN_START_Y + 9), REG_INPUT_DSW3
	INPUT_ENTRY (SCREEN_START_Y + 10), REG_INPUT_SYSTEM
	INPUT_LIST_END

d_screen_xys_list:
	XY_STRING (SCREEN_START_X + 5), (SCREEN_START_Y + 2), "76543210"
	XY_STRING (SCREEN_START_X + 2), (SCREEN_START_Y + 3), "P1"
	XY_STRING (SCREEN_START_X + 2), (SCREEN_START_Y + 4), "P2"
	XY_STRING (SCREEN_START_X + 1), (SCREEN_START_Y + 5), "IN2"
	XY_STRING (SCREEN_START_X + 1), (SCREEN_START_Y + 6), "IN3"
	XY_STRING SCREEN_START_X, (SCREEN_START_Y + 7), "DSW1"
	XY_STRING SCREEN_START_X, (SCREEN_START_Y + 8), "DSW2"
	XY_STRING SCREEN_START_X, (SCREEN_START_Y + 9), "DSW3"
	XY_STRING (SCREEN_START_X + 1), (SCREEN_START_Y + 10), "SYS"
	XY_STRING SCREEN_START_X, SCREEN_B2_Y, "B2+RIGHT - RETURN TO MENU"
	XY_STRING_LIST_END
