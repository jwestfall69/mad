...
video_dac_test:
		...
		; Game only has 4 palettes for fixed tiles use, so
		...
		; green palette setup
		ldx	#TILE_PALETTE + PALETTE_SIZE
		ldd	#$03e0
		jsr	palette_color_setup

		; blue palette setup
		ldx	#TILE_PALETTE + (PALETTE_SIZE*2)
		ldd	#$7c00
		jsr	palette_color_setup

		; combined/all palette setup
		ldx	#TILE_PALETTE + (PALETTE_SIZE*3)
		ldd	#$7fff
		jsr	palette_color_setup
...
full_screen:
		...
		;  word[VD_TILE_NUM_COLORS][VD_TILE_NUM_BITS_PER_COLOR]
		; where:
	...
	.draw_full_screen:
		...
		; (2 bytes per tile * VD_TILE_NUM_BITS_PER_COLOR) = 12
		lda	r_color_num
		lsla			; *2
