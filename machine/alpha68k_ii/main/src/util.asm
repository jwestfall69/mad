	...
	global set_tile_bank
	global r_tile_current_bank

	section code

; Per info in mame, tile bank switching is handled by
; an 74ls259 latch.
...
;
; The tile bank is set by adjusting the latch output
...
;  bit6: bbb = 110
;
; Setting the tile bank will effect all tiles on the
...
;  d0 = bank number
set_tile_bank:
		move.b	(r_tile_current_bank), d1
		cmp.b	d0, d1
		beq	.skip_tile_bank_switch

		move.b	d0, (r_tile_current_bank)

		lea	REG_LATCH_BANK_BIT0, a0
		btst	#$0, d0
		beq	.set_tile_bank_bit0
		add.w	#$40, a0
	.set_tile_bank_bit0:
		move.b	d0, (a0)
	...
	.set_tile_bank_bit1:
		...
		btst	#$2, d0
		beq	.set_tile_bank_bit2
		add.w	#$40, a0
	.set_tile_bank_bit2:
		move.b	d0, (a0)

	.skip_tile_bank_switch:
		rts

	section bss
	align 1

r_tile_current_bank:	dcb.b 1
